 ...
 * @subpackage drivers
 */
class modInstallDriver_pgsql extends modInstallDriver {
    /**
    ...
    public function getCollation() {
        ...
        /*$stmt = $this->xpdo->query("SHOW LC_COLLATE");
    ...
    public function getCollations($collation = '') {
        ...
        if (empty($collation)) $collation = $this->getCollation();
            $col['selected'] = ' selected="selected"';
            $col['value'] = $collation;
            $col['name'] = $collation;
            $collations[$collation] = $col;
    ...
    public function verifyServerVersion() {
        $pgVersion = $this->xpdo->getAttribute(PDO::ATTR_SERVER_VERSION);
        return array('result' => 'success', 'message' => $this->install->lexicon('sqlsrv_version_client_success'),'version' => $pgVersion);
    ...
    public function verifyClientVersion() {
        return array('result' => 'success','message' => $this->install->lexicon('sqlsrv_version_success'),'version' => '');
    ...
    public function addIndex($table,$name,$column) {
        return 'ALTER TABLE '.$this->xpdo->escape($table).' ADD CONSTRAINT '.$this->xpdo->escape($name).' ('.$this->xpdo->escape($column).')"';
    }
    ...
    public function dropIndex($table,$index) {
        return 'ALTER TABLE '.$this->xpdo->escape($table).' DROP CONSTRAINT '.$this->xpdo->escape($index);
    }
